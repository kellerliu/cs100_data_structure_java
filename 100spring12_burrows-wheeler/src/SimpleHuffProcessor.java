...
public class SimpleHuffProcessor implements IHuffProcessor {
	...
	private int writeCompressed(BitInputStream bitsin, BitOutputStream bitsOut) throws IOException{
		int bitCount = 0; 
		
		// write compressed file
		int curWord = bitsin.readBits(BITS_PER_WORD);
		while (curWord != -1) {
			bitCount += writeCompFile(curWord, bitsOut);	
	...
	public int compress(InputStream in, OutputStream out, boolean force)
		...
		if ((mySavedBits <= 0) && (force == false)) {
			...
			// Write pseudo EOF
			myCompBits += writeCompFile(PSEUDO_EOF, bos);
			bos.flush();
			if (myCompBits % 8 != 0)
				myCompBits = (myCompBits / 8 + 1) * 8;
			return myCompBits;
	...
	protected int writeCompFile(int word, BitOutputStream out) {
	...
	private int untransform(BitInputStream bis, OutputStream out) throws IOException{
	...
	private int doOriginalHuffUncompressAfterMagic(BitInputStream bitsIn, BitOutputStream bitsOut) throws IOException{
	...
	public int uncompress(InputStream in, OutputStream out) throws IOException {
		...
		else throw new IOException("magic number not right");
	...
	public int transform(InputStream in, OutputStream out) throws IOException {
		...
		while (true){
			...
			BitInputStream comp = new BitInputStream(new ByteArrayInputStream(array));  //construct a BitInputStream from a ByteArrayInputStream
